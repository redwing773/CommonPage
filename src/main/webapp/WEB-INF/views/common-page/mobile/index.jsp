    ...
    <c:choose>
        <c:when test="${activeProfile eq 'local'}">
            <link rel="stylesheet" type="text/css"
                  href="${webpack:getManifestResource('css/portalPage/m_style.css')}"/>
        </c:when>
        <c:when test="${activeProfile eq 'live'}">
            <c:set var="timestamp" value="<%=new java.util.Date().getTime()%>"/>
            <link rel="stylesheet" type="text/css"
                  href="https://resource-portal.etoos.com/common-page/css/m_style.css?${timestamp}"/>
        </c:when>
        <c:otherwise>
            <c:set var="timestamp" value="<%=new java.util.Date().getTime()%>"/>
            <link rel="stylesheet" type="text/css"
                  href="https://dev-resource-portal.etoos.com/common-page/css/m_style.css?${timestamp}"/>
        </c:otherwise>
    </c:choose>
</head>
<body>
